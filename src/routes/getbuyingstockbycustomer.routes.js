...
router.post('/getBuyingStockByCustomerByDate', async (req, res) => {
  try {
    const { from_date, to_date, search } = req.body || {};
    ...
    const pipeline = [
      ...
      // 4️⃣ 🔍 SINGLE SEARCH POINT (BUYER OR VARIETY)
      ...(safeSearch
        ? [{
            $match: {
              $or: [
                { buyer_name: { $regex: safeSearch, $options: "i" } },
                { "details.variety_details.variety": { $regex: safeSearch, $options: "i" } }
              ]
            }
          }]
        : []),

      // 5️⃣ MERGE FARMER + VARIETY + RATE (NO DUPLICATES)
      {
        $group: {
          _id: {
            buyer_contact: "$buyer_contact",
          ...
          bags: { $sum: "$details.variety_details.bags" },
          weight: { $sum: { $toDouble: "$details.variety_details.weight" } }
        }
      },

      // 6️⃣ BUILD VARIETY DETAILS
      {
        $group: {
          _id: {
            buyer_contact: "$buyer_contact",
